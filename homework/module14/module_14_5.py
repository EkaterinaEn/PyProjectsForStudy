...
from homework.module14 import crud_functions
...
async def get_buying_list(message):
    ...
    for product_item in all_products:
        await message.answer(f'Название: {product_item[1]}| Описание: {product_item[2]}| Цена: {product_item[3]}')
        with open(f'img/{product_item[0]}.jpg', 'rb') as img:
...
async def set_age(message, state):
    ...
    user_d = await state.get_data()
    crud_functions.add_user(user_d['username'], user_d['email'], user_d['age'])
    await message.answer(f'Пользователь {user_d["username"]} зарегистрирован.')
    await state.finish()
